...
class MasaCtrlEditor:
    def __init__(self, method_list, device, num_ddim_steps=50) -> None:
        ...
        self.model = MasaCtrlPipeline.from_pretrained(
            "./models/stable-diffusion-v1-5/", scheduler=self.scheduler).to(device)
    ...
    def __call__(self,
        ...
        if edit_method == "ddim+masactrl":
            return self.edit_image_ddim_MasaCtrl(image_path, prompt_src, prompt_tar, guidance_scale, step=step,
                                                 layper=layper)
        elif edit_method == "directinversion+masactrl":
            return self.edit_image_directinversion_MasaCtrl(image_path, prompt_src, prompt_tar, guidance_scale,
                                                            step=step, layper=layper)
        else:
            raise NotImplementedError(f"No edit method named {edit_method}")

    def edit_image_directinversion_MasaCtrl(self, image_path, prompt_src, prompt_tar, guidance_scale, step=4,
                                            layper=10):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--edit_method_list', nargs='+', type=str,
                        default=["directinversion+masactrl"])  # the editing methods that needed to run
    ...
    masactrl_editor = MasaCtrlEditor(edit_method_list,
    ...
    for key, item in editing_instruction.items():
        ...
        # present_image_save_path = os.path.join(output_path,edit_method,)
        # present_image_save_path = image_path.replace(data_path,
