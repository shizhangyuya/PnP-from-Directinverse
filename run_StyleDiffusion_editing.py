...
stable = StableDiffusionPipeline.from_pretrained('CompVis/stable-diffusion-v1-4', scheduler=scheduler,
...
def text2image_ldm_stable(
    ...
    for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:])):
        ...
        latents = ptp_utils_v.diffusion_step(model, controller, latents, context, t, guidance_scale,
...
def run_and_display(stable, prompts, trainer, controller, latent=None, run_baseline=False, generator=None,
                    verbose=True):
    if run_baseline:
...
def edit_image_stylediffusion_p2p(
    ...
    image_inv_recon, x_t_recon = run_and_display(stable, [prompt_src, prompt_tar], trainer, controller,
    ...
    out_image = Image.fromarray(
        np.concatenate((image_instruct, image_gt[0], image_inv_recon[0], image_inv_edit[1],), axis=1))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--edit_method_list', nargs='+', type=str,
                        default=["stylediffusion+p2p"])  # the editing methods that needed to run
